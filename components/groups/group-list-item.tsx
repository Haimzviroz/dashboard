import { Box, Card, CardContent, Icon, Stack, Typography, IconButton, Tooltip, Dialog, DialogTitle, DialogContent, DialogActions, Button } from "@mui/material"
...
const GroupItem: FC<GroupItemProps> = ({ group, groupsData, selectedGroup, setSelectedGroup }) => {
  ...
  return (
    <Tooltip title={dropReason || ""} open={Boolean(dropReason && isOver && !canDrop)}>
      <Card
        ref={node => {
          drag(node);
          drop(node);
